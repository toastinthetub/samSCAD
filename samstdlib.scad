...
module hex_spacer(height=10, wall=2, clearance=0.2, flat=12.7) {
    r_inner = (flat + clearance) / sqrt(3);     // inner hex radius
    r_outer = (flat + 2*wall) / sqrt(3);         // outer hex radius

    difference() {
        cylinder(h=height, r=r_outer, $fn=6);    // outer hex
        cylinder(h=height + 0.1, r=r_inner, $fn=6);  // inner hex hole
    }
}
